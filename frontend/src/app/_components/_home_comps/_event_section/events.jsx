      ...
      <div className="grid grid-cols-1 grid-rows-1">
        ...
        <main className="grid grid-cols-1 sm:grid-cols-2 grid-rows-[auto] gap-y-10 max-w-7xl row-start-1 col-start-1 mx-auto px-4 py-8 md:px-16 md:py-12">
          ...
          {visibleEvents.map((event) => (
            <main key={event.id} className="grid grid-cols-1 grid-rows-[auto] mx-12 sm:mx-auto sm:px-4" onPointerEnter={() => setActiveId(event.id)} onPointerLeave={() => setActiveId(null)} onPointerDown={() => setActiveId((prev) => (prev === event.id ? null : event.id))} onFocus={() => setActiveId(event.id)} onBlur={() => setActiveId(null)} tabIndex={0}>
